...
struct LocalWorkspacePatcher: Sendable {
    ...
    init(workspaceRoot: URL, fuzzLines: Int = 3) {
        self.workspaceRoot = workspaceRoot.standardizedFileURL
        var p = UnifiedDiffPatcher()
        p.fuzzLines = fuzzLines
        self.patcher = p
    }
    ...
    private func createFile(_ operation: PatchOperation) throws -> PatchResult {
        ...
        let content: String
        if diff.contains("@@") {
            // Has unified diff format — apply in create mode.
            content = try patcher.apply(diff: diff, to: "", mode: .create)
        } else {
            // Plain text — write directly (AI sometimes sends raw content).
            content = diff
        }
    ...
    private func updateFile(_ operation: PatchOperation) throws -> PatchResult {
        ...
        guard diff.contains("@@") else {
            throw PatchToolError.invalidDiff("Update requires a unified diff with @@ hunk headers")
        }

        let patched = try patcher.apply(diff: diff, to: original, mode: .patch)
...
enum ApplyPatchToolDefinition {
    ...
    static func definition() -> OpenAIResponsesToolDefinition {
        OpenAIResponsesToolDefinition(
            name: "apply_patch",
            description: """
                Apply structured file patches using unified diff format. Supports three operations:
                - "create": Create a new file. The diff should contain only addition lines (prefixed with +), \
                or just the raw file content.
                - "update": Modify an existing file using a unified diff. The diff MUST include @@ hunk headers \
                and context lines for verification. Use `read_file_chunk` first to get current file contents.
                - "delete": Remove a file. No diff needed.
                
                This is safer than execute_command for file modifications because:
                1. Paths are validated against the workspace boundary
                2. Context lines verify you're editing the right content
                3. Changes are atomic (write-to-temp then rename)
                4. The operation is structured and auditable
                
                For updates, ALWAYS read the file first with read_file_chunk to get accurate context lines.
                """,
            parameters: .object([
                "type": .string("object"),
                "properties": .object([
