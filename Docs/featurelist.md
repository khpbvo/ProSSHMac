...
- Task alignment (2026-03-08, terminal copy-out still fails despite visible selection):
  - Starting Point: users can drag-select terminal text and invoke Copy via right-click or Cmd+C, but the clipboard often stays unchanged because the copy path still reads the renderer's lossy GPU-oriented snapshot data. That path dropped grapheme-side-table text entirely, could not explicitly identify wide-character continuation cells, and reused raw unclamped selection bounds instead of the normalized selection the highlight renderer actually shows on screen.
  - End Point: terminal copy now extracts from normalized selection bounds against a text-preserving snapshot, with explicit wide-continuation markers and grapheme overrides carried through snapshot publishing, so visible selections copy the same text users see in the terminal instead of returning nil/empty.
- Task alignment (2026-03-08, custom bold-text color integration):
  - Starting Point: terminal rendering already supported bold font weight and the hardcoded `boldIsBright` ANSI behavior, but there was no persisted iTerm-style "Use custom color for bold text" setting. Users could not apply one override color to bold cells without mutating parser/grid color semantics.
  - End Point: Settings now expose a persisted bold-text color toggle + picker, and the Metal renderer applies that override live in the fragment shader for any cell carrying the bold attribute while leaving VT parsing, stored cell colors, and existing ANSI brightening behavior unchanged when the feature is off.
- Task alignment (2026-03-07, terminal scroll still jittery / unresponsive under slow gestures):
  - Starting Point: despite earlier scroll fixes, smooth scrolling still felt jittery and hesitant because small precise-scroll deltas could be partially spring-snapped away between events, making slow scroll gestures under-respond while faster gestures felt inconsistent.
  - End Point: direct precise-scroll gestures now keep their fractional accumulator alive for the full gesture, spring-back only resumes after gesture end, smooth-scroll settings reload at gesture start, and slow + fast scrolling both drive the same continuous path without bleeding away input mid-gesture.
- Task alignment (2026-03-07, TUI alternate-screen scrolling corrupts rendering):
  - Starting Point: when a live TUI / alternate-screen app was actively repainting, wheel scrolling could fight the viewport scrollback path, producing jitter, transient double-rendered frames, and an unusable attempt to scroll up through content that should not be mixed with emulator scrollback. Follow-up diagnosis showed a second bug in the renderer contract: grid scroll-region row remaps only marked the region edges dirty, so partial GPU uploads could leave stale interior rows visible during app-owned scrolling.
  - End Point: emulator viewport scrolling is suppressed while the alternate screen is active, published scroll state is forced back to live-bottom with zero visible scrollback in alt-buffer mode, and grid scroll-region remaps now mark the full affected region dirty so TUI-owned scrolling updates every remapped row instead of leaving ghosted/doubled content on screen.
- Task alignment (2026-03-07, TUI live-scroll artifacts persist after alt-buffer fixes):
  - Starting Point: even after disabling emulator viewport scrollback in the alternate buffer and broadening dirty ranges for row remaps, live TUIs such as Claude Code still show jitter/ghosted mixed frames while the user scrolls and the app continues repainting.
  - End Point: TUI-driven scroll/redraw sequences publish only coherent screen states to the renderer, so wheel-driven app redraws do not expose intermediate erase/repaint phases as visible mixed frames.
- Task alignment (2026-03-08, alternate-screen redraws need quiescent snapshot publishing):
  - Starting Point: `SessionShellIOCoordinator` still fed parser batches every 4 ms while `TerminalRenderingCoordinator` published snapshots on a first-chunk 8/16 ms cadence, so alternate-screen TUIs that redraw across multiple writes but do not fully wrap each frame in DECSET 2026 synchronized output could still show valid-but-intermediate erase/repaint states as jitter, ghosting, or doubled rows during active scrolling.
  - End Point: alternate-screen redraws now publish on a short quiescent debounce window with a bounded max deferral, so split TUI frame writes collapse into one coherent snapshot while main-buffer/live-shell publishing keeps the old responsive cadence.
- Task alignment (2026-03-08, sustained alternate-screen redraws still force mid-frame publishes):
  - Starting Point: the first quiescent-publish fix solved short split redraws, but alternate-buffer redraws still shared the prompt-redraw 50 ms max deferral. Claude Code-style TUI redraw bursts that kept arriving every ~20 ms could therefore age past the cap and force an intermediate publish mid-frame, bringing back jitter/ghosted double-drawn content during active scrolling.
  - End Point: alternate-buffer redraws now have their own longer bounded deferral window, so sustained multi-chunk TUI frame bursts continue coalescing instead of force-publishing after ~50 ms, while shell prompt redraws keep the shorter prompt-specific cap for snappy command-line updates.
- Task alignment (2026-03-08, local shell prompt redraw publishes a blank intermediate line):
  - Starting Point: in local zsh sessions, changing directories could leave the next prompt row visually blank until the user pressed Enter again. Investigation showed the real shell environment emits a temporary reverse-video `%`, erases it with `\r \r`, then prints the real prompt, so main-buffer publishing could still expose that blank intermediate row even outside alternate-screen TUIs.
  - End Point: OSC 133 command-end / prompt-start prompt redraw bursts now reuse the quiescent snapshot debounce path, so shell prompt cleanup + redraw sequences publish one settled prompt frame instead of a transient blank line.
- Task alignment (2026-03-08, intermittent prompt-art glyph corruption in Metal renderer):
  - Starting Point: prompt art such as Claude Code's orange crab/logo could render correctly in some runs and break into missing/misaligned blocks in others, even with identical terminal content. The corruption was intermittent and tracked cache state rather than any specific prompt sequence.
  - End Point: glyph atlas page selection now survives the full CPU-to-GPU path, so glyphs resolve and sample from their actual atlas page instead of implicitly falling back to page 0; prompt art and other glyph-heavy output render deterministically regardless of atlas churn.
- Task alignment (2026-03-08, startup glyph rasterization races poison prompt-art cache):
  - Starting Point: even after preserving atlas page indices, Claude Code's crab/logo could still start in a broken state on some launches. The remaining failure tracked startup timing: glyph misses could rasterize while the renderer still held placeholder font state, and an older background glyph task could finish after a font/atlas rebuild and insert stale glyph bitmaps into the refreshed cache.
  - End Point: glyph rasterization now waits for real font-manager state before dispatching miss jobs, and stale background raster results are discarded when the font/atlas generation changes, so startup timing no longer poisons the prompt-art glyph cache.
- Task alignment (2026-03-08, block-element prompt art still renders with the wrong glyph face):
  - Starting Point: after fixing atlas-page loss and startup raster races, Claude Code's splash still rendered as the right codepoints in the wrong visual shapes. Parser validation showed the grid already contained the exact expected block-element strings, so the remaining fault lived entirely in renderer glyph selection/sampling.
  - End Point: block-element codepoints now bypass font glyph fallback entirely and rasterize from deterministic terminal-cell geometry, so prompt art no longer depends on CoreText choosing compatible fallback faces for Unicode block characters.
- Task alignment (2026-03-08, alternate-buffer TUI flood output still feels delayed and wheel scrolling lags mode changes):
  - Starting Point: after fixing frame coherence and block glyph rendering, heavy alternate-buffer output in TUIs such as Claude Code could still feel a beat behind because each publish also ran expensive shell-buffer extraction, and wheel scrolling could be swallowed right after mouse-mode changes because `inputModeSnapshotsBySessionID` only refreshed when the debounced snapshot publish completed.
  - End Point: parser batches now refresh input-mode snapshots immediately, so mouse-reporting overlays react before the debounced alt-buffer publish, and active alternate-buffer publishes stay on the fast path by skipping shell-buffer extraction/history observation work that is only needed for main-buffer shell content.
- Task alignment (2026-03-08, synchronized-output confirmation redraws can lose their captured frame):
  - Starting Point: Claude Code confirmation overlays such as lingering yes/no prompts could disappear in the terminal's logical grid but remain visible on screen until some later output overwrote them. Raw capture showed the teardown path uses synchronized-output redraws; when a sync-exit snapshot arrived while another publish was still in flight, `TerminalRenderingCoordinator` dropped the snapshot override and the later follow-up publish re-snapshotted from the live engine state instead.
  - End Point: sync-exit snapshot overrides now survive in-flight publishes and are reused by the forced follow-up flush, so synchronized redraws publish the exact cleared frame that Claude emitted instead of a later cached/live substitute.
- Task alignment (2026-03-08, first live publish after synchronized redraw still leaves stale selector rows):
  - Starting Point: after preserving sync-exit snapshot overrides, repeated Claude Code model switches could still leave the old selector UI ghosted on the lower rows while the top `/model` summary updated correctly. That pattern indicates the post-sync live snapshot was current, but its narrow dirty range let old GPU rows survive from the previous synchronized frame.
  - End Point: the first ordinary publish after a synchronized redraw now forces a full snapshot upload, so any stale rows from the old synchronized frame are replaced even when the live post-sync update only dirties the top portion of the grid.
- Task alignment (2026-03-08, repeated Claude `/model` redraw bursts overwrite the full-clear sync snapshot before publish):
  - Starting Point: raw `claude` capture for repeated `/model` confirms one confirm action emits multiple synchronized-output windows in a single parser burst: first a full `CSI 2J` / `CSI 3J` / `CSI H` redraw, then later narrow footer/status redraws. `TerminalGrid` only retained one `syncExitSnapshot`, so the earlier full-clear frame could be overwritten by a later partial snapshot before `SessionShellIOCoordinator` drained it, leaving stale selector rows visible in ProSSH until other output arrived.
  - End Point: synchronized-output capture now queues every sync-exit snapshot produced within a parser batch, and the shell I/O coordinator preserves that correctness by coalescing the batch to its latest frame while forcing a full upload so earlier full-screen clears still land without replaying obsolete intermediate states.
- Task alignment (2026-03-08, long-lived synchronized-output windows can freeze Claude until the next keypress):
  - Starting Point: Claude Code sometimes leaves mode 2026 active across a longer-running plan/thinking redraw burst, which made ProSSH keep showing the cached pre-sync frame. Because `SessionShellIOCoordinator` skipped `scheduleParsedChunkPublish` whenever `engine.synchronizedOutput` stayed true, the UI could look stuck indefinitely until a later keypress caused another redraw boundary.
  - End Point: synchronized-output batches now have a bounded live-snapshot fallback path, so if Claude keeps 2026 active the renderer still publishes a debounced current frame after a short watchdog delay instead of waiting forever for the next sync exit.
- Task alignment (2026-03-08, long-running synchronized redraw bursts replay obsolete TUI frames):
  - Starting Point: after queueing every sync-exit snapshot, a single 4 ms parser batch could contain several already-completed Claude Code redraw frames. `SessionShellIOCoordinator` published each captured snapshot serially, so the renderer visibly replayed older UI states that had already passed before finally landing on the newest frame.
  - End Point: sync-exit snapshots captured in the same parser batch now collapse to the latest frame before publish, and that coalesced sync publish still forces a full upload so batch-local full clears cannot leave stale rows behind.
- Task alignment (2026-03-10, heavy live TUI output still replays slowly and can stop short of bottom):
  - Starting Point: during large Claude Code/TUI redraw floods, `TerminalRenderingCoordinator.publishLatestGridState` marked a follow-up publish when newer output arrived mid-publish, but then tried to recurse into that follow-up before clearing `publishInFlightSessionIDs`. The recursive call immediately saw the session as still in-flight, re-queued the same follow-up, and returned, which left the newest frame unpublished until later activity and made the UI appear to replay buffered output in slow motion.
  - End Point: active publishes now drain queued follow-up work in-place before clearing the in-flight flag, and explicit flushes consume an already-queued follow-up bit before publishing their captured override, so heavy TUI output lands on the newest frame immediately instead of stopping one publish behind.
- Task alignment (2026-03-08, live TUI sessions still need scrollback while alternate-screen output is active):
  - Starting Point: the previous TUI hardening intentionally zeroed published scroll state and no-op'd `scrollTerminal` / `scrollToRow` whenever the alternate buffer was active. That avoided older redraw artifacts, but after the later renderer/input-mode/quiescent-publish fixes it left users unable to scroll up at all while Claude Code or similar TUIs were still running.
  - End Point: entering the alternate buffer still resets any stale pre-TUI scroll position, but once the TUI is live users can wheel up into emulator scrollback, keep that offset through ongoing alternate-buffer publishes, and wheel back down to return to live bottom; wheel-down at live bottom can still fall through to app-owned mouse handling when appropriate.
- Task alignment (2026-03-07, post-unlock delayed terminal catch-up investigation):
  - Starting Point: users report that after the Mac screen saver / lock-unlock path, long-running terminal or remote sessions can appear delayed and then "catch up" rapidly on unlock; a seconds-style runtime counter inside the session output/UI advances unusually fast during the catch-up window.
  - End Point: terminal output ingestion remains live while the app is inactive, but parsed-output UI publishing is suspended and collapsed to latest-state-only semantics; on returning active, stale pending publish tasks are discarded and each dirty session performs a single fresh catch-up snapshot publish instead of replaying delayed intermediate frames.
...
- 2026-03-08: Fixed the remaining long-running Claude Code/TUI “replay” artifact in synchronized-output rendering. Root cause: after the earlier multi-window sync capture fix, one 4 ms parser batch could still contain several already-completed sync-exit frames, and `SessionShellIOCoordinator` published every captured snapshot serially. That preserved clears, but it also replayed obsolete UI states that had already passed before the renderer landed on the newest frame. Fix: added `TerminalRenderingCoordinator.publishSyncExitSnapshots(...)` to coalesce each drained sync batch to its latest frame, while keeping the existing forced-full-upload semantics so earlier full-screen clears in that same batch still replace stale rows. Updated the shell I/O parser loop to use the coalesced path and added `SessionManagerRenderingPathTests.testMultipleSyncExitSnapshotsFromOneParserBatchCollapseToLatestFrame` to lock the regression. Validation: `xcodebuild -project ProSSHMac.xcodeproj -scheme ProSSHMac -destination 'platform=macOS' test -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests` passed (16 tests).
- 2026-03-08: Fixed the remaining “wheel scroll is stuck to live bottom while Claude Code/TUI output keeps rendering” regression by revisiting the alternate-buffer policy itself. Follow-up diagnosis showed the earlier main-buffer mouse-overlay fix was incomplete: the stronger blocker was `TerminalRenderingCoordinator` intentionally zeroing `scrollOffset` and `scrollbackCount` and refusing `scrollTerminal` / `scrollToRow` whenever `usingAlternateBuffer` was true. That policy made sense before the later dirty-range, input-mode, and quiescent-publish fixes, but now it was the direct reason users still could not scroll up during live alternate-screen TUIs. Fix: replaced the hard alt-buffer lockout with directional wheel routing plus entry-only reset semantics. `TerminalSurfaceView` / `ExternalTerminalWindowView` now decide wheel ownership per event: primary-buffer sessions always route wheel to viewport scrollback; alternate-buffer sessions route wheel-up (and any wheel while already scrolled back) into viewport scrollback, while wheel-down at live bottom can still remain app-owned. `TerminalRenderingCoordinator` no longer zeroes scroll state on every alt-buffer publish or scroll request; instead it resets scroll offset only on the main-to-alt buffer transition, preserves scrolled-back offsets through live alt-buffer publishes, and keeps the real scrollback count visible in scroll state. Added/updated regressions: `MouseInputCaptureViewTests` still lock viewport wheel accumulation, and `SessionManagerRenderingPathTests.testAlternateBufferResetsOnEntryButAllowsViewportScrollbackAfterward` now proves alt-buffer entry resets stale offset, users can scroll back while the TUI is active, and live alt-buffer publishes no longer snap them back to bottom. Validation: `xcodebuild -project ProSSHMac.xcodeproj -scheme ProSSHMac -destination 'platform=macOS' build` passed; `xcodebuild -project ProSSHMac.xcodeproj -scheme ProSSHMac -destination 'platform=macOS' test -only-testing:ProSSHMacTests/MouseInputCaptureViewTests -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testAlternateBufferResetsOnEntryButAllowsViewportScrollbackAfterward` passed.
- 2026-03-08: Added iTerm-style custom bold-text color support. Root cause: bold rendering was split between font-weight selection and an optional hardcoded ANSI brightening path (`boldIsBright`), with no renderer-level override for users who want all bold text drawn in one custom color. Fix: introduced persisted `BoldTextColorConfiguration`, added a Settings screen for toggling/picking the custom bold color, reloaded that configuration live through `MetalTerminalSessionSurface`, threaded it into `TerminalUniformBuffer`, and updated `TerminalShaders.metal` so any cell with the bold attribute uses the configured foreground override at render time. VT parsing and stored grid colors remain unchanged when the feature is disabled. Added `BoldTextColorConfigurationTests` for defaults + persistence round-trip. Validation: `xcodebuild -project ProSSHMac.xcodeproj -scheme ProSSHMac -destination 'platform=macOS' build` passed, and `xcodebuild -project ProSSHMac.xcodeproj -scheme ProSSHMac -destination 'platform=macOS' test -only-testing:ProSSHMacTests/BoldTextColorConfigurationTests` passed.
- 2026-03-08: Confirmed the Claude Code splash regression is downstream of the parser and hardened block-element glyph selection. Added `VTParserTests.testClaudeCodeSplashLayoutUsesExpectedBlockCharacters`, which feeds Claude's real startup sequence (DECSET 2026 + block-element art + cursor-forward CSI) and proves the grid already contains the correct splash rows before Metal renders them. Follow-up renderer fix: `resolveRenderFont` now special-cases Unicode Block Elements (`U+2580...U+259F`) to prefer a stable Menlo fallback when available, instead of trusting primary fonts that technically contain the glyphs but render poor/stylized block geometry. Validation: `swiftc -parse ProSSHMac/Terminal/Renderer/MetalTerminalRenderer+GlyphResolution.swift` passed, and `xcodebuild -project ProSSHMac.xcodeproj -scheme ProSSHMac -destination 'platform=macOS' test -only-testing:ProSSHMacTests/GlyphIndexPackingTests -only-testing:ProSSHMacTests/VTParserTests/testClaudeCodeSplashLayoutUsesExpectedBlockCharacters` passed.
- 2026-03-08: Replaced the remaining font-dependent Claude Code splash rendering with deterministic block-element rasterization. Follow-up investigation showed the parser was already correct and CoreText still chose mixed fallback families for the splash glyphs (`▐▛███▜▌`, `▝▜█████▛▘`, `▘▘ ▝▝`), so the renderer could improve slightly without ever becoming stable while it relied on font glyph shapes for `U+2580...U+259F`. Fix: `GlyphRasterizer` now detects Unicode Block Elements and draws their halves/eighths/quadrants directly into the atlas scratch buffer instead of going through CoreText; this makes those cells independent of the selected terminal font or fallback face. Added regressions in `GlyphRasterizerTests` to prove block elements rasterize identically across fonts and occupy the expected quadrants/halves, while keeping `VTParserTests.testClaudeCodeSplashLayoutUsesExpectedBlockCharacters` as the parser-level contract. Validation: `swiftc -parse ProSSHMac/Terminal/Renderer/GlyphRasterizer.swift ProSSHMacTests/Terminal/Tests/GlyphRasterizerTests.swift` passed, and `xcodebuild -project ProSSHMac.xcodeproj -scheme ProSSHMac -destination 'platform=macOS' test -only-testing:ProSSHMacTests/GlyphRasterizerTests -only-testing:ProSSHMacTests/VTParserTests/testClaudeCodeSplashLayoutUsesExpectedBlockCharacters` passed.
- 2026-03-08: Fixed the remaining alternate-buffer responsiveness gap after the redraw/glyph work. Root cause had two parts: (1) `inputModeSnapshotsBySessionID` was only refreshed inside `publishGridState`, so mouse-tracking/encoding changes made by a TUI could lag behind the debounced alt-buffer publish and leave the wheel temporarily swallowed by the Metal view instead of reaching the mouse-report overlay; (2) every alt-buffer publish still ran `visibleText()` + history observation work intended for main-buffer shell content, adding avoidable latency during flood output. Fix: `SessionShellIOCoordinator` now refreshes input-mode snapshots through `TerminalRenderingCoordinator` immediately after each parsed batch, and `publishGridState` now skips shell-buffer extraction/history observation while the alternate buffer is active. Added regressions in `SessionManagerRenderingPathTests` for immediate input-mode refresh ahead of the debounce window and for keeping alt-buffer publishes off the shell-buffer path. Validation: `swiftc -parse ProSSHMac/Services/TerminalRenderingCoordinator.swift ProSSHMac/Services/SessionShellIOCoordinator.swift ProSSHMacTests/Terminal/Tests/SessionManagerRenderingPathTests.swift` passed, and `xcodebuild -project ProSSHMac.xcodeproj -scheme ProSSHMac -destination 'platform=macOS' test -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testAlternateBufferInputModeRefreshesBeforeDebouncedPublish -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testAlternateBufferPublishDoesNotRewriteShellBuffer -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testAlternateBufferSplitRedrawPublishesOnlyAfterQuiescentWindow -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testAlternateBufferSustainedRedrawDoesNotForceIntermediatePublishAtFiftyMilliseconds` passed.
- 2026-03-08: Fixed a synchronized-output follow-up publish race behind lingering Claude Code confirmation overlays. Raw `claude` capture showed model/confirmation teardown frames are emitted inside DECSET 2026 redraw bursts, with a sync-exit snapshot available before later footer/status updates reopen synchronized output. `TerminalRenderingCoordinator` previously dropped that snapshot override whenever a publish was already in flight, and a follow-up review showed stale overrides could also survive after being used once and then get replayed by a later ordinary follow-up publish. Fix: added per-session pending snapshot override storage, preserved sync-exit overrides across in-flight publishes, threaded those overrides through `flushPendingSnapshotPublishIfNeeded` and recursive follow-up publishes, cleared the stored override during session cleanup, and now discard any older pending override as soon as a real publish starts so unrelated later updates cannot replay it. Added `SessionManagerRenderingPathTests.testSyncExitSnapshotOverrideSurvivesInFlightPublishAndWinsFollowUpFlush` to lock the regression. Validation: `swiftc -parse ProSSHMac/Services/TerminalRenderingCoordinator.swift ProSSHMacTests/Terminal/Tests/SessionManagerRenderingPathTests.swift` passed, and `xcodebuild -project ProSSHMac.xcodeproj -scheme ProSSHMac -destination 'platform=macOS' test -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testSyncExitSnapshotOverrideSurvivesInFlightPublishAndWinsFollowUpFlush -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testAlternateBufferInputModeRefreshesBeforeDebouncedPublish -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testAlternateBufferPublishDoesNotRewriteShellBuffer -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testAlternateBufferSplitRedrawPublishesOnlyAfterQuiescentWindow -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testAlternateBufferSustainedRedrawDoesNotForceIntermediatePublishAtFiftyMilliseconds` passed.
- 2026-03-10: Fixed the remaining heavy-output TUI replay regression where the terminal could animate buffered redraws in slow motion and still stop above the newest frame until the user scrolled again. Root cause: `publishLatestGridState` used a `defer` to clear `publishInFlightSessionIDs`, but it also tried to recurse into any queued follow-up publish before that defer ran. The recursive call therefore saw the session as still in flight, re-set the follow-up bit, and returned, leaving the newest frame unpublished until a later event forced another flush. A related follow-up-flush edge case kept the queued bit set even when `flushPendingSnapshotPublishIfNeeded` was already explicitly consuming that request, which could trigger an unnecessary second live publish after a sync-exit override. Fix: converted follow-up draining to an in-place loop inside `publishLatestGridState`, clear the queued follow-up bit before explicit flushes, and added a deterministic debug-only follow-up injection hook for regression coverage. Added/updated tests: `SessionManagerRenderingPathTests.testFollowUpPublishRequestedDuringActivePublishDrainsBeforeReturn`, `SessionManagerRenderingPathTests.testSyncExitSnapshotOverrideSurvivesInFlightPublishAndWinsFollowUpFlush`, and `SessionManagerRenderingPathTests.testMultipleSyncExitSnapshotsFromOneParserBatchCollapseToLatestFrame`. Validation: `xcodebuild -project ProSSHMac.xcodeproj -scheme ProSSHMac -destination 'platform=macOS' test -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testFollowUpPublishRequestedDuringActivePublishDrainsBeforeReturn -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testSyncExitSnapshotOverrideSurvivesInFlightPublishAndWinsFollowUpFlush -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testMultipleSyncExitSnapshotsFromOneParserBatchCollapseToLatestFrame` passed.
- 2026-03-08: Fixed the remaining repeated `/model` selector ghosting by preserving every synchronized-output frame inside a parser batch instead of only the last one. Reproduced the issue against the real `claude` 2.1.71 TTY: confirming `/model` emits one sync window with a full `CSI 2J` / `CSI 3J` / `CSI H` redraw, followed by later sync windows for the `High /model` footer and status line. `TerminalGrid` previously stored only one `syncExitSnapshot`, so later narrow captures could overwrite the earlier full-clear frame before `SessionShellIOCoordinator` consumed it. Fix: replaced the single snapshot slot with a queued `syncExitSnapshots` array, exposed `consumeSyncExitSnapshots()` through `TerminalEngine`, and updated `SessionShellIOCoordinator` to publish every queued snapshot in order after each parser batch. Added `TerminalGridTests.testSynchronizedOutput_ConsumeSyncExitSnapshotsReturnsQueuedUnsyncedFrames` to lock the multi-window capture contract. Validation: `swiftc -parse ProSSHMac/Terminal/Grid/TerminalGrid.swift ProSSHMac/Terminal/Grid/TerminalGrid+ModeSetters.swift ProSSHMac/Terminal/Parser/TerminalEngine.swift ProSSHMac/Services/SessionShellIOCoordinator.swift ProSSHMacTests/Terminal/Tests/TerminalGridTests.swift` passed, and `xcodebuild -project ProSSHMac.xcodeproj -scheme ProSSHMac -destination 'platform=macOS' test -only-testing:ProSSHMacTests/TerminalGridTests/testSynchronizedOutput_CapturesUnsyncedFrameOnEnable -only-testing:ProSSHMacTests/TerminalGridTests/testSynchronizedOutput_ConsumeSyncExitSnapshotsReturnsQueuedUnsyncedFrames -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testSyncExitSnapshotOverrideSurvivesInFlightPublishAndWinsFollowUpFlush -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testFirstLivePublishAfterSyncExitForcesFullSnapshotUpload` passed.
- 2026-03-08: Fixed the remaining “Claude looks frozen until I press Enter” path by adding a bounded synchronized-output live-frame fallback. Investigation showed `SessionShellIOCoordinator` intentionally skipped ordinary snapshot scheduling whenever `engine.synchronizedOutput` stayed true, which is correct for short redraw windows but leaves the UI apparently stuck if Claude keeps mode 2026 open for a longer-running thinking/status update. Fix: refactored `TerminalGrid` snapshot generation so `snapshot()` still respects the sync cache while new `liveSnapshot()` can intentionally bypass it, exposed that through `TerminalEngine`, and added a debounced `.synchronizedOutput` publish mode in `TerminalRenderingCoordinator`. The shell I/O path now schedules that fallback with the live snapshot whenever a batch ends still inside sync mode, so the UI advances after a short watchdog delay without waiting for the next keypress. Added regressions `TerminalGridTests.testSynchronizedOutput_LiveSnapshotBypassesCachedFrame` and `SessionManagerRenderingPathTests.testSynchronizedOutputFallbackPublishesLiveSnapshotWithoutWaitingForSyncExit`. Validation: `swiftc -parse ProSSHMac/Terminal/Grid/TerminalGrid+Snapshot.swift ProSSHMac/Terminal/Parser/TerminalEngine.swift ProSSHMac/Services/TerminalRenderingCoordinator.swift ProSSHMac/Services/SessionShellIOCoordinator.swift ProSSHMacTests/Terminal/Tests/TerminalGridTests.swift ProSSHMacTests/Terminal/Tests/SessionManagerRenderingPathTests.swift` passed, and `xcodebuild -project ProSSHMac.xcodeproj -scheme ProSSHMac -destination 'platform=macOS' test -only-testing:ProSSHMacTests/TerminalGridTests/testSynchronizedOutput_CapturesUnsyncedFrameOnEnable -only-testing:ProSSHMacTests/TerminalGridTests/testSynchronizedOutput_ConsumeSyncExitSnapshotsReturnsQueuedUnsyncedFrames -only-testing:ProSSHMacTests/TerminalGridTests/testSynchronizedOutput_LiveSnapshotBypassesCachedFrame -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testSyncExitSnapshotOverrideSurvivesInFlightPublishAndWinsFollowUpFlush -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testFirstLivePublishAfterSyncExitForcesFullSnapshotUpload -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testSynchronizedOutputFallbackPublishesLiveSnapshotWithoutWaitingForSyncExit` passed.
- 2026-03-08: Fixed terminal copy-out so visible selections reliably reach the macOS clipboard again. Root cause: `selectedText()` still extracted from the renderer's compact `GridSnapshot` cells as if `glyphIndex` alone were sufficient text storage, but snapshot generation discarded grapheme-side-table content and had no explicit marker for wide-character continuation cells. The copy path also re-used the raw stored selection bounds instead of re-clamping/re-expanding them the same way the on-screen selection renderer does. Fix: `GridSnapshot` now carries per-cell grapheme overrides plus a `wideContinuation` cell attribute for width-0 cells, snapshot generation preserves that metadata for both live-grid and scrollback snapshots, and the copy path now delegates to a normalized text extractor that clamps bounds, honors word/line expansion, skips continuation cells, and reconstructs grapheme text from the preserved overrides. Added focused regressions in `TerminalSelectionCopyTests` for grapheme + wide-char extraction and for out-of-bounds word-selection normalization. Validation: `xcodebuild -project ProSSHMac.xcodeproj -scheme ProSSHMac -destination 'platform=macOS' test -only-testing:ProSSHMacTests/TerminalSelectionCopyTests` passed, and `xcodebuild -project ProSSHMac.xcodeproj -scheme ProSSHMac -destination 'platform=macOS' build` passed.
- 2026-03-08: Fixed the remaining intermittent Claude Code logo corruption after the atlas-page fix. Root cause: the Metal renderer started with placeholder startup font state (`rasterFontName = "SF Mono"`, 8x16 cells) and could queue background glyph raster jobs before `initializeFontMetricsAndPrepopulate()` finished syncing the real `FontManager` state. On systems where the eventual font resolved differently (for example to Menlo), those old jobs could still complete after the atlas/cache rebuild and insert stale glyph bitmaps into the fresh cache, leaving block-element prompt art in a mixed/wrong font until later churn replaced it. Fix: added renderer font-state readiness + glyph-raster generation tracking, block miss-job dispatch until real font state is ready, cancel/drop stale background glyph jobs across font/atlas rebuilds, and clear pending miss sets during reload. Validation: `swiftc -parse ProSSHMac/Terminal/Renderer/MetalTerminalRenderer.swift ProSSHMac/Terminal/Renderer/MetalTerminalRenderer+FontManagement.swift ProSSHMac/Terminal/Renderer/MetalTerminalRenderer+DrawLoop.swift` passed, and `xcodebuild -project ProSSHMac.xcodeproj -scheme ProSSHMac -destination 'platform=macOS' test -only-testing:ProSSHMacTests/GlyphIndexPackingTests` passed.
- 2026-03-08: Fixed intermittent glyph corruption for prompt art and other atlas-heavy output in the Metal renderer. Root cause: glyph resolution tracked `AtlasEntry.atlasPage`, but the packed `glyphIndex` sent to the GPU only preserved X/Y coordinates and `TerminalShaders.metal` always sampled atlas texture page 0. When cache churn pushed some glyphs onto later atlas pages, the renderer read texels from the wrong page, so logos such as Claude Code's orange crab broke nondeterministically depending on atlas state. Fix: introduced shared glyph-index packing that encodes atlas page + X/Y coordinates, updated the vertex/fragment shaders to decode the page and sample from the correct atlas texture array slot, and bound all atlas pages during the terminal scene pass. Added `GlyphIndexPackingTests` to lock page-preserving packing/unpacking behavior. Validation: `xcodebuild -project ProSSHMac.xcodeproj -scheme ProSSHMac -destination 'platform=macOS' test -only-testing:ProSSHMacTests/GlyphIndexPackingTests` passed.
- 2026-03-08: Fixed the remaining sustained alt-buffer redraw leak after the first quiescent-publish pass. Root cause: `TerminalRenderingCoordinator` still used one shared 50 ms max deferral for both prompt redraws and alternate-screen redraws. That was fine for shell prompt cleanup bursts, but too short for Claude Code-style TUI redraws that arrived every ~20 ms; once a pending alt-buffer publish aged past 50 ms, the coordinator stopped rescheduling and published a valid-but-intermediate frame mid-redraw. Fix: split parsed-output debounce handling into `.alternateBuffer` and `.promptRedraw` modes, kept prompt redraws on the short 24 ms / 50 ms path, and raised the alternate-buffer max deferral to 150 ms so longer TUI redraw bursts keep coalescing before the renderer sees them. Added `SessionManagerRenderingPathTests.testAlternateBufferSustainedRedrawDoesNotForceIntermediatePublishAtFiftyMilliseconds` to lock the regression. Validation: `xcodebuild -project ProSSHMac.xcodeproj -scheme ProSSHMac -destination 'platform=macOS' test -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testAlternateBufferSplitRedrawPublishesOnlyAfterQuiescentWindow -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testAlternateBufferSustainedRedrawDoesNotForceIntermediatePublishAtFiftyMilliseconds -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testSemanticPromptRedrawPublishesOnlyAfterQuiescentWindow -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testSemanticPromptEventsArmQuiescentPublishBeforeScheduling` passed. The broader `SessionManagerRenderingPathTests` suite still has the pre-existing unrelated `testLocalSessionStreamsProgressiveCommandOutput` failure in this environment.
- 2026-03-08: Fixed the ordering bug in the local-shell prompt redraw debounce. Follow-up investigation showed the first prompt fix still had a race: `SessionManager.configureHistoryTracking` forwarded OSC 133 semantic prompt events into `TerminalRenderingCoordinator` through a detached `Task { @MainActor ... }`, so `scheduleParsedChunkPublish` could run before `promptRedrawPendingSessionIDs` was set. That let the first post-command publish stay on the normal 8 ms cadence and reintroduced the “blank prompt line until the next Enter” symptom for real local zsh redraws. Fix: changed the semantic prompt handler to `await` the main-actor `noteSemanticPromptEvent(...)` call inline before returning to the parser/publish path. Added `SessionManagerRenderingPathTests.testSemanticPromptEventsArmQuiescentPublishBeforeScheduling` to verify that scheduling immediately after OSC 133 `commandEnd`/`promptStart` markers is already inside the quiescent window. Validation: targeted prompt tests pass via `xcodebuild -project ProSSHMac.xcodeproj -scheme ProSSHMac -destination 'platform=macOS' -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testSemanticPromptRedrawPublishesOnlyAfterQuiescentWindow -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testSemanticPromptEventsArmQuiescentPublishBeforeScheduling test`. The unrelated `testLocalSessionStreamsProgressiveCommandOutput` still fails in this environment and remains the next open local-session issue.
- 2026-03-08: Fixed local shell prompt blank-line redraws after `cd`. Root cause: the user’s zsh environment emits a short prompt-cleanup sequence before the real prompt (`reverse-video %`, then `\r \r` erase, then OSC 133 command-end / prompt-start markers and the final prompt text). `TerminalRenderingCoordinator` previously treated that like ordinary main-buffer output and could publish the erased intermediate row before the settled prompt landed, matching the “blank line until next Enter” report. Fix: added `promptRedrawPendingSessionIDs` in `TerminalRenderingCoordinator`, wired OSC 133 semantic prompt events from `SessionManager.configureHistoryTracking`, and routed command-end/prompt-start redraw bursts through the same bounded quiescent debounce used for alt-screen redraws. Added `SessionManagerRenderingPathTests.testSemanticPromptRedrawPublishesOnlyAfterQuiescentWindow` to lock the behavior. Validation: `swiftc -parse` passed for the modified app/test files; targeted `xcodebuild -project ProSSHMac.xcodeproj -scheme ProSSHMac -destination 'platform=macOS' -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests/testSemanticPromptRedrawPublishesOnlyAfterQuiescentWindow test` passed. Note: the pre-existing `testLocalSessionStreamsProgressiveCommandOutput` is currently failing in this environment and remains an outstanding local-session validation gap.
- 2026-03-08: Fixed the remaining alternate-screen TUI frame-atomicity leak. Root cause: the previous alt-buffer fixes stopped emulator scrollback from fighting the app, but `TerminalRenderingCoordinator` still scheduled parsed-output publishes on the first chunk of each burst. With `SessionShellIOCoordinator` batching every 4 ms, TUIs such as Claude Code could redraw one logical frame across multiple writes and the renderer would publish a mid-frame state before the redraw settled. Fix: `scheduleParsedChunkPublish` is now async, detects alternate-buffer sessions, and routes them through a quiescent debounce path in `TerminalRenderingCoordinator` (24 ms debounce, 50 ms max deferral) that reschedules pending publishes until the redraw burst settles, while leaving the existing 8/16 ms cadence for normal shell output. Added `SessionManagerRenderingPathTests.testAlternateBufferSplitRedrawPublishesOnlyAfterQuiescentWindow` to lock the behavior. Validation: `swiftc -parse` passed for the modified app/test files, and `xcodebuild -project ProSSHMac.xcodeproj -scheme ProSSHMac -destination 'platform=macOS' -only-testing:ProSSHMacTests/SessionManagerRenderingPathTests test` now passes (7 tests, 0 failures). Also fixed two pre-existing test-harness compile blockers in `TerminalGridTests.swift` (`String` vs `Character` mismatch in the new dirty-range regression helper) so the targeted suite could build.
- 2026-03-07: Deep-dive diagnosis for the remaining TUI scroll artifact. Evidence now points away from the viewport scrollback path and toward frame atomicity in the TUI output pipeline. `TerminalSurfaceView` places `MouseInputHandler` above the Metal surface, so once a TUI enables mouse tracking the wheel is consumed by `MouseInputCaptureView.scrollWheel()` and sent into the app, not into emulator scrollback. The remaining corruption therefore comes from the redraw path after those wheel events. `SessionShellIOCoordinator` batches raw output on a fixed 4 ms timer and `TerminalRenderingCoordinator` publishes snapshots every 8/16 ms without any alternate-screen-specific quiescence or atomic frame boundary beyond DECSET 2026 synchronized output. That means a TUI that redraws in multiple writes but does not fully wrap each redraw in synchronized output can expose valid-but-intermediate parser states as visible frames, which matches the reported jitter / ghosted duplicate content that disappears on subsequent output. No code change in this step; this is the current leading root-cause hypothesis pending implementation.
- 2026-03-07: Fixed the remaining TUI render ghosting during live app-owned scrolling. The first alternate-buffer patch correctly disabled emulator viewport scrollback, but the screenshot follow-up showed the real corruption was deeper in the grid-to-renderer contract: `TerminalGrid.scrollUp/scrollDown` remap every row in the scroll region yet previously marked only `scrollTop` and `scrollBottom` dirty. Because `CellBuffer` uses partial GPU uploads when `GridSnapshot.dirtyRange` is narrow, stale middle rows from older frames could survive and appear as transient double/ghosted output while TUIs such as Claude Code scrolled internally. Fix: added contiguous dirty-range marking in `TerminalGrid+TabsAndDirty.swift` and changed `TerminalGrid+Scrolling.swift` to mark the full scroll region dirty after every up/down remap. Added `TerminalGridTests.testPartialScrollRegionMarksEntireDirtyRangeForSnapshot` to lock the snapshot contract. Verification: `swiftc -parse` passed for the modified grid/test files; full `xcodebuild` remains blocked here by the missing Metal toolchain component.
- 2026-03-07: Fixed TUI alternate-screen scroll corruption. Root cause: the renderer/UI still allowed emulator viewport scrolling and published nonzero scrollback state while the terminal was on the alternate screen, even though continuously repainting TUIs (`vim`, `htop`, etc.) should not be mixed with scrollback snapshots. This could produce jitter and transient double images as live alt-screen redraws fought scrolled viewport snapshots. Fixes: `TerminalMetalView` now gates viewport wheel scrolling behind an `allowsViewportScrolling` provider; both embedded and external terminal surfaces disable that path whenever the current `GridSnapshot` is using the alternate buffer; `TerminalRenderingCoordinator` now no-ops `scrollTerminal` / `scrollToRow` in alt-buffer mode and publishes `scrollOffset = 0`, `scrollbackCount = 0` while alt-screen is active. Added regression coverage in `SessionManagerRenderingPathTests` for alternate-buffer scroll suppression. Verification: `swiftc -parse` passed for modified app/test files; full `xcodebuild` remains blocked here by the missing Metal toolchain component.
- 2026-03-07: Improved smooth-scroll responsiveness for slow and fast gestures. Root cause: `SmoothScrollEngine` used the same `renderOffset` as both the visible sub-row offset and the input accumulator, while `frame()` spring-snapped that value toward zero every frame even during an active gesture. Small precise-scroll deltas could therefore bleed off between events, producing jittery/hesitant scrolling. Fix: added explicit gesture lifecycle state to `SmoothScrollEngine` (`beginGesture` / `endGesture`), disabled spring-back while a direct gesture is active, and treated active gestures as requiring continuous frames. `TerminalMetalContainerView` now detects precise gesture start/end, reloads smooth-scroll settings at gesture start, and routes gesture/momentum transitions through `MetalTerminalRenderer`. Follow-up: qualified the direct-gesture momentum check to `NSEvent.Phase.none` in `TerminalMetalView.swift` to remove the Swift warning about comparing a non-optional phase to `Optional.none`. Added regression tests covering slow-gesture accumulation across frames and post-gesture spring-back. Verification: `swiftc -parse` passed for modified renderer/test files; full `xcodebuild` remains blocked in this environment by the missing Metal toolchain component.
- 2026-03-07: Implemented the first post-unlock catch-up fix pass. `TerminalRenderingCoordinator` now suspends parsed-output snapshot publishing while the app is inactive/backgrounded, marks sessions dirty instead of draining queued intermediate publishes, collapses follow-up publish requests to a latest-state-only bit while a publish is already in flight, resets burst-mode state on suspend/resume, and performs one immediate catch-up publish per dirty session on `applicationDidBecomeActive()`. `SessionManager` now routes inactive/background/active lifecycle events into the rendering coordinator, and `ProSSHMacApp` forwards `scenePhase == .inactive` as well. Added regression coverage in `SessionManagerRenderingPathTests` for (1) no nonce/shell-buffer publish while inactive and (2) multiple deferred publishes collapsing into a single catch-up nonce on resume. Verification: `swiftc -parse` passed for the modified Swift files; full `xcodebuild build` / targeted tests are currently blocked in this environment because the machine is missing the Metal toolchain (`xcodebuild` fails at `CompileMetalFile` with “cannot execute tool 'metal' due to missing Metal Toolchain”).
- 2026-03-07: Investigated the reported "fast counter after unlock" behavior without changing code. Current leading hypothesis is not a pure elapsed-time bug but delayed terminal output ingestion/render replay after unlock: `SessionShellIOCoordinator.startParserReader()` uses an unbounded async stream plus 4 ms batching (`SessionShellIOCoordinator.swift`), and parsed output then hops through `@MainActor` rendering/snapshot publishing with additional 8/16 ms coalescing (`TerminalRenderingCoordinator.swift`). The app currently reacts only to coarse `scenePhase` active/background changes (`ProSSHMacApp.swift`) and has no explicit App Nap / screen-sleep wake handling recorded in the repo, making occlusion/background throttling a likely contributor for both local and remote sessions.
