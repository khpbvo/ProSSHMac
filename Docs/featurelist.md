...
- Task alignment (2026-03-08, repeated Claude `/model` redraw bursts overwrite the full-clear sync snapshot before publish):
  ...
  - End Point: synchronized-output capture now queues every sync-exit snapshot produced within a parser batch, and the shell I/O coordinator drains/publishes them in order so the renderer always sees the earlier full-screen clear before later narrow footer/status updates.
